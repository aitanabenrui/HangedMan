...
const randomWord = ()=>{
    const words = ['gorra','viento', 'nieve', 'fresa', 'camisa',
        'dinero', 'tomates', 'colegio', 'barriga', 'zumo', 'reloj', 'camiseta', 'cuadrado', 'abrigo',
        'libro', 'verde', 'rojo', 'amarillo', 'azul', 'perro', 'gato', 'animal', 'moco', 'manzana', 'arbol', 'lechuga', 'zapato',
        'tabaco', 'armario', 'anime', 'mesa', 'comida', 'ballena', 'rodilla', 'mano', 'pie', 'cabza', 'calabaza', 'zapatilla' ];
...
const wordInput = (e)=>{ // representa el evento que en este caso es submit, el botón de enviar
...
const hangedMan = (letter, button) =>{ //le estoy pasando la letara de cada botón y el botón en sí,
    
    //esta parte del código es la que hace que al apretar un botón con letra, se ponga o no en el array
    if (wordArray.includes(letter)){ //si el array definido en la función wordInput contiene la letra que hemos pulsado
        wordArray.forEach((char, index)=>{ //iniciamos un forEach para recorrer el array
            if(char === letter){ //en el caso de que una de las letras del array coincida con la letra pulsada
        ...
        if(counterLetters == wordArray.length){ //si el contador de letras es igual al largo de wordArray significará que han acertado la palabra entera y habrán ganado
            setTimeout(()=>{ //esta función hace lo mismo que la de más abajo, la de fail, pero con la imágen win porque la condición es que ha ganado
                ...
                buttons.forEach(btn =>{ //ppara cada botón se obtiene su texto y se eliminan los espacios ectra con trim
